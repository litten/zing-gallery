...
function _getAlbums(files, staticFilesPath, pathFromReq, json){
  ...
  files = _.map(files, function(albumName){
    return {
      ...
      name : albumName,
      password: config.albums[albumName].password,
      passwordTips: config.albums[albumName].passwordTips || 'password needed'
    };
  });
  return files;
}
...
function sortByTime (files, albumPath) {
  ...
  for (var i = 0, len = files.length; i < len; i++) {
    for (var j = i + 1; j < len; j++) {
      ...
      var compare
      if (sortByTime) {
        compare = (formatName(files[i].exif.Time) > formatName(files[j].exif.Time))
      } else {
        compare = (formatName(files[i].exif.Time) < formatName(files[j].exif.Time))
      }
      if (hasExif && compare) {
